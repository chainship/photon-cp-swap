...
fn load_cfg(client_config: &String) -> Result<ClientConfig> {
    ...
    let photon_cp_program_str = config.get("Global", "photon_cp_program").unwrap();
    if photon_cp_program_str.is_empty() {
        panic!("photon_cp_program must not be empty");
    }
    let photon_cp_program = Pubkey::from_str(&photon_cp_program_str).unwrap();
...
fn main() -> Result<()> {
    ...
    let program = anchor_client.program(pool_config.photon_cp_program)?;
    ...
    match opts.command {
        RaydiumCpCommands::InitializePool {
            ...
            let initialize_pool_instr = initialize_pool_instr(
                ...
                photon_cp_swap::create_pool_fee_reveiver::ID,
                random_pool_id,
                init_amount_0,
                init_amount_1,
                open_time,
        ...
        RaydiumCpCommands::Deposit {
            ...
            let pool_state: photon_cp_swap::states::PoolState = program.account(pool_id)?;
            // load account
            ...
            // docode account
            let pool_state =
                utils::deserialize_anchor_account::<photon_cp_swap::states::PoolState>(
            ...
            let (total_token_0_amount, total_token_1_amount) = pool_state
                ...
                .unwrap();
            // calculate amount
            let results = photon_cp_swap::curve::CurveCalculator::lp_tokens_to_trading_tokens(
                u128::from(lp_token_amount),
                u128::from(pool_state.lp_supply),
                u128::from(total_token_0_amount),
                u128::from(total_token_1_amount),
                photon_cp_swap::curve::RoundDirection::Ceiling,
            )
            .ok_or(photon_cp_swap::error::ErrorCode::ZeroTradingTokens)
            .unwrap();
            println!(
        ...
        RaydiumCpCommands::SwapBaseIn {
            ...
            let pool_state =
                ...
                .unwrap();
            let amm_config_state = deserialize_anchor_account::<photon_cp_swap::states::AmmConfig>(
            ...
            let (
                ...
                (
                    photon_cp_swap::curve::TradeDirection::ZeroForOne,
                ...
                (
                    photon_cp_swap::curve::TradeDirection::OneForZero,
                    total_token_1_amount,
                    total_token_0_amount,
                    user_input_token,
                    spl_associated_token_account::get_associated_token_address(
            ...
            let result = photon_cp_swap::curve::CurveCalculator::swap_base_input(
        ...
        RaydiumCpCommands::SwapBaseOut {
            ...
            let result = photon_cp_swap::curve::CurveCalculator::swap_base_output(
        ...
        RaydiumCpCommands::DecodeEvent { log_event } => {
            handle_program_log(
                &pool_config.photon_cp_program.to_string(),
                &log_event,
                false,
            )?;
        }
        RaydiumCpCommands::DecodeTxLog { tx_id } => {
            ...
            // decode logs
            parse_program_event(&pool_config.photon_cp_program.to_string(), meta.clone())?;
        }
    }
    Ok(())
}
